...
#define nsqlcFree g_free
#define nsqlcMalloc g_malloc
#define nsqlcRealloc g_realloc
...
static void mout(void *arg, char *zNewText, int nNewChar){
  ...
  if( pM->nChar + nNewChar + 1 > pM->nAlloc ){
    ...
    if( pM->zText==pM->zBase ){
      pM->zText = nsqlcMalloc(pM->nAlloc);
      if( pM->zText && pM->nChar ) memcpy(pM->zText,pM->zBase,pM->nChar);
    }else{
      char *z = nsqlcRealloc(pM->zText, pM->nAlloc);
      if( z==0 ){
        nsqlcFree(pM->zText);
        pM->nChar = 0;
        pM->nAlloc = 0;
...
/*
** nsqlc_mprintf() works like printf(), but allocations memory to hold the
** resulting string and returns a pointer to the allocated memory.  Use
** nsqlcFree() to release the memory allocated.
*/
char *nsqlc_mprintf(const char *zFormat, ...){
  va_list ap;
  ...
  if( sMprintf.zText!=sMprintf.zBase ){
    nsqlcFree(sMprintf.zText);
  }
  return zNew;
}

/* This is the varargs version of nsqlc_mprintf.  
*/
char *nsqlc_vmprintf(const char *zFormat, va_list ap){
  struct sgMprintf sMprintf;
  char *zNew;
  char zBuf[200];
  sMprintf.nChar = 0;
...
** nsqlc_exec() and nsqlc_get_table() interfaces.  See the nsqlc.h
...
int nsqlc_exec_printf(
  nsqlc *db,                   /* An open database */
  const char *sqlFormat,        /* printf-style format string for the SQL */
  nsqlc_callback xCallback,    /* Callback function */
  ...
  va_start(ap, errmsg);
  rc = nsqlc_exec_vprintf(db, sqlFormat, xCallback, pArg, errmsg, ap);
  va_end(ap);
  return rc;
}
int nsqlc_exec_vprintf(
  ...
  zSql = nsqlc_vmprintf(sqlFormat, ap);
  rc = nsqlc_exec(db, zSql, xCallback, pArg, errmsg);
  free(zSql);
  return rc;
}
#if 0
int nsqlc_get_table_printf(
  ...
  va_start(ap, errmsg);
  rc = nsqlc_get_table_vprintf(db, sqlFormat, resultp, nrow, ncol, errmsg, ap);
  va_end(ap);
  return rc;
}
int nsqlc_get_table_vprintf(
  ...
  zSql = nsqlc_vmprintf(sqlFormat, ap);
  rc = nsqlc_get_table(db, zSql, resultp, nrow, ncolumn, errmsg);
  free(zSql);
  return rc;
}
#endif
