...
static unsigned int uncompress_DOC
    (
    unsigned char*  src,     /* in:  compressed document */
    unsigned int    src_len, /* in:  size of compressed document */
    unsigned char*  dest,    /* out: buffer to put uncompressed document in */
...
static unsigned int uncompress_ZLib
    (
    unsigned char*       src,     /* in:  compressed document */
    unsigned int         src_len, /* in:  size of compressed document */
    unsigned char*       dest,    /* out: buffer to put uncompressed document in */
...
static void free_plucker_doc (plkr_Document* doc) {
  if (doc->name != NULL)
    free (doc->name);
  if (doc->title != NULL)
    free (doc->title);
...
static int GetUncompressedRecord (plkr_Document *doc, plkr_DBHandle handle,
                                  int record_index, unsigned char *buffer, int buffer_size,
                                  plkr_DataRecordType expected_type,
                                  unsigned char **buffer_out, int *buffer_size_out,
                                  plkr_DataRecord **record_out)
{
  ...
  int size_needed, blen = buffer_size;
  ...
  if (!record->cache) {
    ...
    if ((record->type == PLKR_DRTYPE_TEXT_COMPRESSED) ||
        ...
        if (!handle->seek(handle, record->offset) ||
            (handle->read(handle, buf, record->size, record->size) != record->size)) {
...
#if 0
        _plkr_message("data record %d (%d):  uid is %d, # paras = %d, size = %d, type = %d",
                  record_index, record->size,
                  (buf[0] << 8) + buf[1],
                  (buf[2] << 8) + buf[3],
                  (buf[4] << 8) + buf[5],
                  buf[6]);
        ...
        if (doc->compression == PLKR_COMPRESSION_ZLIB) {
          if (uncompress_ZLib(start_of_data, len_of_data, output_ptr, buf_to_use, (doc->owner_id_required ? doc->owner_id_key : NULL)) != Z_OK) {
          ...
          if (uncompress_DOC(start_of_data, len_of_data, output_ptr, buf_to_use) != 1) {
        ...
        if (!handle->seek(handle, record->offset) ||
            (handle->read(handle, tbuffer, blen, size_needed) != size_needed)) {
            _plkr_message("Bad read from DBHandle while reading record %d", record->uid);
            if (tbuffer != buffer) free(tbuffer);
            return FALSE;
          }
...
static int ParseCategories (plkr_Document *newdoc, plkr_DBHandle handle)
{
  ...
  struct _plkr_CategoryName *categories, *newc;
  int bufsize;

  if (GetUncompressedRecord(newdoc, handle, newdoc->default_category_record_uid,
                            NULL, 0, PLKR_DRTYPE_CATEGORY, &buf, &bufsize, &record)) {
...
static int ParseMetadata (plkr_Document *newdoc, plkr_DBHandle handle)
{
  unsigned char *buf, *ptr;
  int bufsize;
  int nsubrecords, typecode, subrecord_length, i;

  if (!GetUncompressedRecord(newdoc, handle, newdoc->metadata_record_uid,
                             NULL, 0, PLKR_DRTYPE_METADATA, &buf, &bufsize, NULL)) {
    return FALSE;
  } else {
...
static int ParseURLs (plkr_Document *newdoc, plkr_DBHandle handle)
{
  ...
  unsigned char *buf, *ptr;
  char **urls;
  int id, i, n, count, nurls, bufsize;
  struct url_index_record { int last_url_index; int record_id; } *records;
  ...
  if (!GetUncompressedRecord(newdoc, handle, newdoc->urls_index_record_uid,
                             NULL, 0, PLKR_DRTYPE_LINKS_INDEX, &buf, &bufsize, NULL)) {
    return FALSE;
  } else {
    n = ((buf[4] << 8) + buf[5]) / 4;
  ...
  for (count = 0, i = 0;  i < n;  i++) {
    ...
    if (!GetUncompressedRecord(newdoc, handle, id,
                               NULL, 0, PLKR_DRTYPE_NONE, &buf, &bufsize, &record)) {
      goto errout4;
    }
    ...
    record->cached_size = bufsize;
    buf = NULL;
    for (ptr = record->cache + 8; (ptr - record->cache) < record->cached_size;  ptr += (strlen(ptr) + 1)) {
#ifdef DEBUGURLS
...
#ifdef DEBUGOPEN
    {
      static char *types[] = { "TEXT", "TEXTC", "IMAGE", "IMAGEC", "MAILTO",
                                 "URLINDEX", "URLS", "URLSC", "BOOKMARKS", "CATEGORIES",
                                 "METADATA" };
    _plkr_message("%3d:  type=%10s, offset=%07x, size=%5d, uncompressed_size=%5d",
              record->uid, types[MIN(record->type, sizeof(types)/sizeof(char *))],
...
static int fp_seek (plkr_DBHandle handle, long offset)
{
  long result;
...
static int fp_read (plkr_DBHandle handle, unsigned char *buffer, int buffersize, int readsize)
{
  int result;
  result = read((int)(handle->dbprivate), buffer, MIN(buffersize, readsize));
  if (result != readsize) {
    _plkr_message("Unable to read %d bytes from fp %d -- read %d instead\n",
              MIN(buffersize, readsize), (int)(handle->dbprivate), result);
  }
  return (result);
}

static void fp_free (plkr_DBHandle handle)
{
...
static long fp_size (plkr_DBHandle handle)
{
  int fp = (int) (handle->dbprivate);
  struct stat buf;
   ...
   plkr_DataRecordType *type)
{
  ...
  if (!GetUncompressedRecord(doc, doc->handle, record_index,
                             output_buffer, output_buffer_size, PLKR_DRTYPE_NONE,
                             NULL, &output_size, &record))
    return 0;
  else {
    *type = record->type;
    return output_size;
