...
static gboolean
has_suffix (char *main_string, char *potential_suffix)
{
    int l1 = strlen (main_string);
    int l2 = strlen (potential_suffix);

    return ((l1 >= l2) && (strncmp (main_string + (l1 - l2), potential_suffix, l2) == 0));
}
...
void
enumerate_library (GTraverseFunc func, gpointer user_data)
{
...
void
get_metadata (int fp, int *nrecords, GSList ** categories_out, int *charset_out, char **title_out, char **author_out, time_t * pubtime_out)
{
    unsigned char databuf[128];
    int nrecs, rec_offset, nreserved, i, type, id, j, metadata_record_index = 0,
      categories_record_index = 0, nrecord_headers, rec_length;
    // int nread;
    unsigned long ctime;
    ...
    for (id = 0, i = 0; i < nreserved; i++)
      {
          ...
          if (type == PLKR_DEFAULT_CATEGORY_NAME)
            {
                categories_record_index = READ_BIGENDIAN_SHORT (databuf + 6 + (i * 4));
            }
          else if (type == PLKR_METADATA_NAME)
            {
                metadata_record_index = READ_BIGENDIAN_SHORT (databuf + 6 + (i * 4));
            }
      }
    ...
    if (categories_record_index > 0 || metadata_record_index > 0)
      {
          for (i = 1; i < MIN (5, nrecord_headers); i++)
            {
                rec_offset = READ_BIGENDIAN_LONG (record_offsets_data + (8 * i));
                rec_length = READ_BIGENDIAN_LONG (record_offsets_data + (8 * (i + 1))) - rec_offset;
                lseek (fp, rec_offset, 0);
                read (fp, databuf, 10);
                id = READ_BIGENDIAN_SHORT (databuf);
                type = *(databuf + 6);
                if (id == categories_record_index && type == PLKR_DRTYPE_CATEGORY)
                  {
                      char *ptr, *temp;
                      j = rec_length - 8;
                      temp = malloc (j);
                      ...
                      for (ptr = temp; ptr < (temp + j);)
                        {
                            categories = g_slist_prepend (categories, g_strdup (ptr));
                            ptr += strlen (ptr) + 1;
                        }
                ...
                else if (id == metadata_record_index && type == PLKR_DRTYPE_METADATA)
                  {
                      ...
                      for (j = 0; j < npieces; j++)
                        {
                            ...
                            if (type == 1)
                              ...
                              {
                                  read (fp, databuf, 4);
                                  *pubtime_out = READ_BIGENDIAN_LONG (databuf) - PLKR_TIMEADJUST;
                              }
                            else
                              {
          ...
          for (i = 0; dirs[i] != NULL; i++)
            {
                ...
                /* handle ~ characters */
                if (dirname[0] == '~' && dirname[1] == G_DIR_SEPARATOR && homedir)
                  {
                      dirname = g_strjoin (G_DIR_SEPARATOR_S, homedir, dirs[i] + 2, NULL);
                  }

                if (!g_file_test (dirname, G_FILE_TEST_EXISTS))
                  {
                      fprintf (stderr, "Non-existent path/file on PLUCKERPATH:  %s\n", dirname);
                      continue;
                  }
                      ...
                      if ((dir = g_dir_open (dirname, 0, NULL)))
                        {
                            ...
                            while ((fname = (char *) g_dir_read_name (dir)) != NULL)
                              {

                                  if (has_suffix (fname, ".plkr") ||
                                      has_suffix (fname, ".pdb"))
                                    {

                                        path = g_strjoin (G_DIR_SEPARATOR_S, dirname, fname, NULL);
                                        fp = open (path, O_RDONLY);
                                        if ((docname = is_plucker (fp)) != NULL)
                                          {
                                              ...
                                              docinfo->path = path;
                                              get_metadata (fp, &docinfo->nrecords,
                                                       &docinfo->categories,
                                                          &docinfo->charset,
                                                            &docinfo->title,
                                                            &docinfo->author,
                  ...
                  {
                      fprintf (stderr, "Non-Plucker, non-directory element on PLUCKERPATH:  %s\n", dirname);
                  }
