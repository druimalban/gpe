...
static int check_essid(gpointer user_data) {
	...
	if (! f) fprintf(stderr, "problem opening /etc/network/interfaces\n");
	else {
...
guint start_connection(gpointer user_data){
    ...
    gchar sys_cmd[128];
    system("ifdown eth0");
    sprintf(sys_cmd,"/usr/bin/net-interfaces %s",gtk_label_get_text(GTK_LABEL(label4)));
    system(sys_cmd);
    system("ifup eth0");
    update_wireless(user_data);
    g_timeout_add_full(G_PRIORITY_DEFAULT,1250,switch_to_status,user_data,NULL);
                                        ...
                                        gpointer         user_data)
{
	...
	gtk_notebook_remove_page(GTK_NOTEBOOK(notebook1),-1);
	gtk_notebook_remove_page(GTK_NOTEBOOK(notebook1),-1);
	g_timeout_add_full(G_PRIORITY_DEFAULT,250,start_connection,user_data,NULL);
    }
