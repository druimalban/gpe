...
GdkPixmap *cell_black_sel;
GdkPixmap *cell_white_sel;

GdkBitmap *cell_empty_mask;
GdkBitmap *cell_black_mask;
GdkBitmap *cell_white_mask;
GdkBitmap *cell_black_sel_mask;
GdkBitmap *cell_white_sel_mask;

GdkPixmap *cells [15];
GdkBitmap *cells_mask [15];
...
void sendCoords (Coords clickedCell)
{
	...
	if (tableau[clickedCell.h][clickedCell.v] == coulact) {
		if (selected.h == clickedCell.h &&
		    selected.v == clickedCell.v) {
			selected = nullCoords;
		} else {
    ...
    short playingColor;
		...
		if ((canPlay(selected, clickedCell, coulact)) || (clickedCell.h == 0)) {
			bouge = 1;
			playingColor = coulact;
...
void initPlateau (GtkWidget *win) {
	...
	gtk_pixmap_get (GTK_PIXMAP (gtk_object_get_data (GTK_OBJECT(win),
									...
									&cell_white,
									&cell_white_mask);
	gtk_pixmap_get (GTK_PIXMAP (gtk_object_get_data (GTK_OBJECT(win), "blackselcell")),
					&cell_black_sel,
					&cell_black_sel_mask);
	gtk_pixmap_get (GTK_PIXMAP (gtk_object_get_data (GTK_OBJECT(win), "whiteselcell")),
					&cell_white_sel,
					&cell_white_sel_mask);
	gtk_pixmap_get (GTK_PIXMAP (gtk_object_get_data (GTK_OBJECT(win),
																									 "emptycell")),
	...
	cells [13] = cell_white_sel;
	cells_mask [13] = cell_white_sel_mask;
	cells [14] = cell_black_sel;
	cells_mask [14] = cell_black_sel_mask;
	...
	for (h=1;h<=8;h++)
		for (v=1;v<=8;v++) {
			...
			if (tableau[h][v] == noir) {
				...
				else if (grilleAffichage[h-1][v-1] > 12 && (selected.h != h || selected.v != v))
					grilleAffichage[h-1][v-1] = 12;
			...
			if (v == selected.v && h == selected.h) {
				if (grilleAffichage[h-1][v-1] < 13) {
					grilleAffichage[h-1][v-1] =
						tableau[h][v] == noir ? 14 : 13;
					changed = 1;
				}
			}
...
prefs (void)
{
  ...
  gtk_window_set_title (GTK_WINDOW (pref_dialog), _("Ataxx: Preferences"));
...
struct gpe_icon my_icons[] = {
  { "new", },
  { "preferences" },
  { "icon", PREFIX "/share/pixmaps/gpe-ataxx.png" },
  { "exit", },
  { NULL }
};
...
main (int argc, char *argv[])
{
  ...
  GtkWidget *whitecell;
  GtkWidget *blackcell;
  GtkWidget *whiteselcell, *blackselcell;
  ...
  gtk_window_set_title (GTK_WINDOW (window), _("Ataxx"));
  ...
  /* Selected White & Black cells */
  whiteselcell = create_pixmap (window, "WSCell.xpm");
  gtk_widget_set_name (whiteselcell, "whiteselcell");
  gtk_widget_ref (whiteselcell);
  gtk_object_set_data_full (GTK_OBJECT (window), "whiteselcell", whiteselcell,
                            (GtkDestroyNotify) gtk_widget_unref);
  gtk_widget_show (whiteselcell);
//  gtk_table_attach (GTK_TABLE (table2), whiteselcell, 0, 1, 1, 2,
//                    (GtkAttachOptions) (GTK_FILL),
//                    (GtkAttachOptions) (GTK_FILL), 1, 0);
  gtk_misc_set_alignment (GTK_MISC (whiteselcell), 0, 0.5);
                                                                                            
  blackselcell = create_pixmap (window, "BSCell.xpm");
  gtk_widget_set_name (blackselcell, "blackselcell");
  gtk_widget_ref (blackselcell);
  gtk_object_set_data_full (GTK_OBJECT (window), "blackselcell", blackselcell,
                            (GtkDestroyNotify) gtk_widget_unref);
  gtk_widget_show (blackselcell);
//  gtk_table_attach (GTK_TABLE (table2), blackselcell, 0, 1, 3, 4,
//                    (GtkAttachOptions) (GTK_FILL),
//                    (GtkAttachOptions) (GTK_FILL), 1, 0);
