...
static const char *contacts_schema_str = "create table contacts (urn INTEGER NOT NULL, tag TEXT NOT NULL, value TEXT NOT NULL);";

/* entry data for identification and absic information */
static const char *contacts_schema2_str = "create table contacts_urn (urn INTEGER PRIMARY KEY, name TEXT, family_name TEXT, company TEXT);";

/* this one is for config data */
static const char *contacts_schema4_str = "create table contacts_config (id INTEGER PRIMARY KEY,	cgroup INTEGER NOT NULL, cidentifier TEXT NOT NULL, cvalue TEXT);";
...
contacts_read_one_entry_old (void *arg, int argc, char **argv, char **names)
{
  ...
  p->id = atoi (argv[0]);
  sqlite_exec_printf (contacts_db, "select value from contacts where (urn=%d) and (tag='NAME')",
                      contacts_read_name, p, NULL, p->id);
  sqlite_exec_printf (contacts_db, "select value from contacts where (urn=%d) and (tag='FAMILY_NAME')",
  ...
  if (r) /* r > 0 indicates a failure, need to recreate that table with new schema */
    {
      ...
      for (iter = entries; iter; iter=iter->next)
        {
           ...
           if (p)
             {
               sqlite_exec_printf(contacts_db, "insert into contacts_urn values (%d, '%q', '%q', '%q')", 
      ...
      r = sqlite_exec (contacts_db, "create index idxtag on contacts (tag)", NULL, NULL, &err);
      if (r)
          goto error;
      ...
      for (iter = list; iter; iter = iter->next)
        {
           ...
           if (strcmp(iter->data, utag))
             {
               r = sqlite_exec_printf (contacts_db, "update contacts set tag='%q' where (tag='%q')", 
...
contacts_db_open (gboolean open_vcard)
{
  ...
  if (sqlite_exec (contacts_db, contacts_schema4_str, NULL, NULL, NULL) == SQLITE_OK)
    {
      contacts_db_add_config_values (CONFIG_PANEL, _("Name"), "NAME");
      contacts_db_add_config_values (CONFIG_PANEL, _("Phone"), "HOME.TELEPHONE");
...
{
  struct contacts_tag_value *t = g_malloc (sizeof (struct contacts_tag_value));
  t->tag = tag;
  t->value = value;
  return t;
}
...
{
  char *err;
  int r = sqlite_exec (contacts_db, "insert into contacts_urn values (NULL, NULL, NULL, NULL)",
		       NULL, NULL, &err);
  if (r)
    {
      gpe_error_box (err);
...
contacts_db_get_entries (void)
{
  ...
  r = sqlite_exec (contacts_db, "select urn, name, family_name from contacts_urn",
  ...
  if (no_name && !no_cat) 
    {
      r = sqlite_exec_printf 
        (contacts_db, "select contacts_urn.urn, contacts_urn.name, contacts_urn.family_name, contacts_urn.company "\
...
contacts_db_get_entries_finddlg (const gchar *str, const gchar *cat)
{
  ...
  if (has_cat && has_str) 
    {
      r = sqlite_exec_printf (contacts_db, "select distinct contacts_urn.urn, contacts_urn.name, contacts_urn.family_name, contacts_urn.company "\
...
contacts_db_get_entries_filtered (const gchar* filter)
{
  ...
  if (filter)
    r = sqlite_exec_printf (contacts_db, "select urn, name, family_name from contacts_urn "\
...
GSList *
contacts_db_get_entries_list_filtered (const gchar* str, const gchar *filter, const gchar *cat)
{
  GSList *list = NULL;
  char *err = NULL;
  int r = 0;
  ...
  if (!has_cat && !has_str) 
    ...
    {
      r = sqlite_exec_printf 
        (contacts_db, "select distinct contacts_urn.urn, contacts_urn.name, contacts_urn.family_name, contacts_urn.company "\
             "from contacts_urn, contacts where contacts_urn.urn = contacts.urn "\
...
contacts_db_get_by_uid (guint uid)
{
  ...
  p->id = uid;
  r = sqlite_exec_printf (contacts_db, "select tag,value from contacts where urn=%d",
...
contacts_db_delete_by_uid (guint uid)
{
  ...
  r = sqlite_exec_printf (contacts_db, "delete from contacts_urn where urn='%d'",
			  NULL, NULL, &err, uid);
  if (r)
    goto error;
...
contacts_db_find_tag (struct contacts_person *p, gchar * tag)
{
  ...
  for (iter = p->data; iter; iter = iter->next)
    {
      struct contacts_tag_value *id = (struct contacts_tag_value *) iter->data;
...
void
contacts_db_set_multi_data (struct contacts_person *p, gchar * tag, gchar * value)
{
  struct contacts_tag_value *t = contacts_new_tag_value (g_strdup (tag), value);
...
contacts_db_get_tag_list (gchar *** list)
{
  ...
  sqlite_get_table (contacts_db, "select distinct tag from contacts", list, &r, &c,
		    &err);
  if (err)
    g_free (err);
  return r;
...
void
contacts_db_update_config_values (gint group, gchar * identifier, gchar * value)
{
