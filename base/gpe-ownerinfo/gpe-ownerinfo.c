...
on_expose_event (GtkWidget *widget, GdkEventExpose *event, gpointer data)
{
  ...
  scaledpixbuf = gdk_pixbuf_scale_simple (pixbuf, resultwidth, resultheight, GDK_INTERP_BILINEAR);

  gdk_pixbuf_render_to_drawable_alpha (scaledpixbuf, widget->window,
				       0, 0,                      /* src_x, src_y */
				       (gint) ((maxwidth - resultwidth)/2), 0, /* dest_x, dest_y */
...
create_GPE_Ownerinfo (void)
{
  ...
  gtk_container_add (GTK_CONTAINER (GPE_Ownerinfo), mainvbox);
  ...
  */
  ...
  gtk_container_add (GTK_CONTAINER (smallphotobutton), drawingarea);
...
main (int argc, char *argv[])
{
  ...
  /* FIXME: check error */
  pixbuf = gdk_pixbuf_new_from_file (photofile, NULL);
