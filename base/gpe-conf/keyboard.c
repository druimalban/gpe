...
get_keyboard_defs ()
{
	...
	while (lines[i])
	{
		...
		//snprintf(buf,strlen(el[0]),"%s",el[0]);
		kbds[i-1].identifier = g_strdup(buf);		
		
		while (el[j])
		{
			if (val = strstr(el[j],"module-package"))
				kbds[i-1].package_name = g_strdup(g_strchomp(strstr(val,"=")+1));
			else if (val = strstr(el[j],"module-name"))
				kbds[i-1].module_name = g_strdup(g_strchomp(strstr(val,"=")+1));
			else if (val = strstr(el[j],"device"))
				kbds[i-1].device_name = g_strdup(g_strchomp(strstr(val,"=")+1));
			else if (val = strstr(el[j],"support-package"))
				kbds[i-1].support_package = g_strdup(g_strchomp(strstr(val,"=")+1));
			j++;
		}
		if (j < 4)
		{
			snprintf(buf,255,"%s: %s",_("Error reading keyboard definition)"),kbds[i].identifier);
			gpe_error_box(buf);
		}
		g_strfreev(el);
		i++;
...
Keyboard_Build_Objects (void)
{
	...
	for (i=0;i<numkbds;i++)
	{
		kbds[i].rbSelect = gtk_radio_button_new_with_label_from_widget (label, kbds[i].identifier);
		label = kbds[i].rbSelect;
		gtk_table_attach (GTK_TABLE (table), kbds[i].rbSelect, 0, 1, i+1, i+2,
		    GTK_FILL,
		    GTK_FILL | GTK_EXPAND, 0, 0);
		gtk_tooltips_set_tip (tooltips, kbds[i].rbSelect,
			      _("Tap here to select this keyboard model."),
			      NULL);
    	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(kbds[i].rbSelect),TRUE);
