...
#define bb_error_msg_and_die(x) {fprintf(strerr, "%s\n", x); return -1}
...
int update_passwd(const struct passwd *pw, char *crypt_pw)
{
	char filename[1024];
	char buf[1025];
	char buffer[80];
	...
	if (access(PATH_SHADOW, F_OK) == 0) {
		has_shadow = 1;
	}
	if (has_shadow) {
		snprintf(filename, sizeof filename, "%s", PATH_SHADOW);
	} else {
		snprintf(filename, sizeof filename, "%s", PATH_PASSWD);
	}
