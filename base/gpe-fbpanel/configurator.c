...
static void
set_align(GtkComboBox *widget, gpointer bp)
{
    int align;
    gboolean t;
    
    ENTER;	
    align = gtk_combo_box_get_active(widget) + 1;
    DBG("align=%d\n", align);
...
mk_position()
{
    ...
    align_opt = gtk_combo_box_new_text();
    gtk_combo_box_append_text(GTK_COMBO_BOX(align_opt), "Left");
    gtk_combo_box_append_text(GTK_COMBO_BOX(align_opt), "Center");
    gtk_combo_box_append_text(GTK_COMBO_BOX(align_opt), "Right");
    g_signal_connect(G_OBJECT(align_opt), "changed", G_CALLBACK(set_align), NULL);
    gtk_widget_show(align_opt);

    hbox = gtk_hbox_new(FALSE, 0);
    gtk_box_pack_start(GTK_BOX (hbox), align_opt, FALSE, TRUE, 0);
    gtk_table_attach(GTK_TABLE(t), hbox, 1, 2, 1, 2, GTK_FILL, 0, 0, 0);                
    gtk_label_set_mnemonic_widget(GTK_LABEL(label), align_opt);
    ...
    gtk_table_set_col_spacing(GTK_TABLE(t), 1, 5);
...
mk_dialog()
{
    ...
    DBG("creating dialog\n");
    dialog = gtk_dialog_new_with_buttons (_("Panel Configurator"),
          NULL,
...
configure(void)
{
    ...
    update_opt_menu(align_opt, p->align - 1);
...
global_config_save(FILE *fp)
{
    ...
    fprintf(fp, "    align = %s\n",
          num2str(align_pair, gtk_combo_box_get_active(GTK_COMBO_BOX(align_opt)) + 1, "none"));
