...
draw_signal (int signal)
{
  ...
  gdk_draw_rectangle (GDK_DRAWABLE (dock_window), gc, TRUE, 0,
		      16 - signal * 30 / 255, 16, signal * 30 / 255);
}
...
get_linklevel (void)
{
  ...
  if ((wireless = fopen ("/proc/net/wireless", "r")) != NULL)
	...
	{
	  sscanf (line, "%*s %*s %d. %d. %*d. %*d %*d %*d", &link, &level);
	}
      fclose (wireless);
    }
  return link;
...
update_netlist (psnetinfo_t * anet)
{
  ...
  if (!found)
	...
	memcpy (&netlist[netcount - 1]->netinfo, snet,
      ...
      memcpy (&netlist[netcount - 1]->net, anet, sizeof (psnetinfo_t));
...
do_message_info (psinfo_t * psi)
{
  ...
  switch (psi->info)
    ...
    case I_FAILED:
      if ((net_request_nr >= 0)
	  && (netlist[net_request_nr]->netinfo.mode == 1))
	gpe_error_box_nonblocking (_("Ad-Hoc connections setup complete."));
      else
      ...
      else
	{
	  switch (msg.type)
	    ...
	    case msg_network:
	      ...
	      break;
	    case msg_config:
	      memcpy (&cfg, &msg.content.cfg, sizeof (psconfig_t));
	      break;
	    case msg_info:
...
list_add_net (netinfo_t * ni)
{
  ...
  gtk_tree_store_set (store, &iter,
		      ...
		      COL_SSID, ni->netinfo.ssid,
		      COL_WEP, ni->netinfo.wep,
		      COL_CHANNEL, ni->netinfo.channel, COL_NET, ni, -1);
