...
gboolean _key_equal( gconstpointer a, gconstpointer b ) {

    return a == b;

}

static void _dictionary_update_lookup_hash( Dictionary* dict ) {
    ...
    for( node = dict->list->next; node; node = node->next ) {
        ...
        if( new_char != current_char ) {
            ...
            g_hash_table_replace( dict->lookup_hash, GINT_TO_POINTER( (int) current_char ), node );
...
gint _string_compare( gconstpointer a, gconstpointer b ) {
...
void dictionary_add_word( Dictionary* dict, const char* word, gboolean rehash ) {
    ...
    if( rehash ) {
        ...
        first_word = g_hash_table_lookup( dict->lookup_hash, GINT_TO_POINTER( (int) word[0] ) );
        if( strcmp( new_word, (char*) first_word->data ) < 0 )
            g_hash_table_replace( dict->lookup_hash, GINT_TO_POINTER( (int) new_word[0] ), g_list_find( dict->list, new_word ) );
...
void dictionary_remove_word( Dictionary* dict, const char* word, gboolean rehash ) {
    ...
    node = g_hash_table_lookup( dict->lookup_hash, GINT_TO_POINTER( (int) word[0] ) ); 
    ...
    if( rehash && strcmp( node->data, word ) == 0 ) {
        ...
        if( node->next && ( (char*) node->next->data)[0] == word[0] ) 
            g_hash_table_replace( dict->lookup_hash, GINT_TO_POINTER( (int) word[0] ), node->next );
        else
            g_hash_table_remove( dict->lookup_hash, GINT_TO_POINTER( (int) word[0] ) );
