 ...
 *
 */
gboolean i_feel_dirty( gpointer data ) {
...
void irc_input_predict_word() {

    int i, j, k, last_pos, current_pos, old_pos;
    char current_char;

    old_pos = current_pos = gtk_editable_get_position( GTK_EDITABLE( irc_input_entry ) );
    ...
    if( predicted_word ) {
        ...
        gtk_editable_insert_text( GTK_EDITABLE( irc_input_entry ), &predicted_word[word_pos], strlen( predicted_word ) - word_pos, &current_pos ); 

       gtk_signal_handler_unblock( irc_input_entry, entry_insert_text_connection_id );
        ...
        g_idle_add_full( G_PRIORITY_HIGH_IDLE + 17, i_feel_dirty, GINT_TO_POINTER( current_pos ), NULL ); 
        g_idle_add_full( G_PRIORITY_HIGH_IDLE + 18, i_feel_dirty, GINT_TO_POINTER( old_pos ), NULL ); 
 ...
 *
 */
GtkListStore* input_popup_create_list_from_file( char* filename ) {
...
gboolean input_popup_list_button_press_event( GtkWidget* widget, GdkEventButton* event, gpointer data ) {
    ...
    if( gtk_tree_view_get_path_at_pos( GTK_TREE_VIEW( widget ), event->x, event->y, &path, NULL, NULL, NULL ) ) {
        ...
        if( gtk_tree_model_get_iter( model, &iter, path ) ) {
            ...
            current_pos = gtk_editable_get_position( GTK_EDITABLE( irc_input_entry ) );
            gtk_signal_handler_block( irc_input_entry, entry_insert_text_connection_id );
            gtk_editable_insert_text( GTK_EDITABLE( irc_input_entry ), smiley , strlen( smiley ), &current_pos ); 
            gtk_signal_handler_unblock( irc_input_entry, entry_insert_text_connection_id );
            gtk_widget_grab_focus( irc_input_entry );
            gtk_editable_select_region( GTK_EDITABLE( irc_input_entry ), current_pos, current_pos );
...
GtkWidget* input_popup( GtkListStore* store ) {
    ...
    gtk_signal_connect( GTK_OBJECT( popup_list ), "button-press-event",
                        GTK_SIGNAL_FUNC( input_popup_list_button_press_event ), popup_window );
    ...
    gtk_window_move( GTK_WINDOW( popup_window ), x_pos, y_pos - popup_window->allocation.height );
...
void irc_input_quick_button_clicked( GtkWidget* widget, gpointer data ) {
...
void irc_input_smiley_button_clicked( GtkWidget* widget, gpointer data ) {
...
void irc_input_set_nick_dictionary( Dictionary* dictionary ) {
