...
** memory as necessary.
*/
static int usqld_get_table_cb(void *pArg, int nCol, char **argv, char **colv){
  ...
  */
  if( p->nRow==0 ){
    ...
    sqliteSetString(&p->zErrMsg,
       "usqld_get_table() called with two or more incompatible queries", 0);
    p->rc = SQLITE_ERROR;
    return 1;
  }
...
int usqld_get_table(
  ...
  *pazResult = 0;
  ...
  res.azResult[0] = 0;
  rc = usqld_exec(db, zSql, usqld_get_table_cb, &res, pzErrMsg);
  if( res.azResult ){
    res.azResult[0] = (char*)res.nData;
  }
  if( rc==SQLITE_ABORT ){
    usqld_free_table(&res.azResult[1]);
    if( res.zErrMsg ){
      if( pzErrMsg ){
        free(*pzErrMsg);
