...
/*
** usqld_mprintf() works like printf(), but allocations memory to hold the
** resulting string and returns a pointer to the allocated memory.  Use
** free() to release the memory allocated.
*/
char *usqld_mprintf(const char *zFormat, ...){
  va_list ap;
...
/* This is the varargs version of usqld_mprintf.  
*/
char *usqld_vmprintf(const char *zFormat, va_list ap){
  struct sgMprintf sMprintf;
  char zBuf[200];
  sMprintf.nChar = 0;
  sMprintf.zText = zBuf;
...
int usqld_exec_printf(
  usqld_conn *db,                   /* An open database */
  const char *sqlFormat,        /* printf-style format string for the SQL */
  usqld_callback xCallback,    /* Callback function */
  ...
  va_start(ap, errmsg);
  rc = usqld_exec_vprintf(db, sqlFormat, xCallback, pArg, errmsg, ap);
  va_end(ap);
  return rc;
}
...
int usqld_exec_vprintf(
  ...
  zSql = usqld_vmprintf(sqlFormat, ap);
  rc = usqld_exec(db, zSql, xCallback, pArg, errmsg);
  free(zSql);
  return rc;
}
int usqld_get_table_printf(
  ...
  va_start(ap, errmsg);
  rc = usqld_get_table_vprintf(db, sqlFormat, resultp, nrow, ncol, errmsg, ap);
  va_end(ap);
  return rc;
}
int usqld_get_table_vprintf(
  ...
  zSql = usqld_vmprintf(sqlFormat, ap);
  rc = usqld_get_table(db, zSql, resultp, nrow, ncolumn, errmsg);
  free(zSql);
  return rc;
}
