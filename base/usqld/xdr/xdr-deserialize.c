...
int  XDR_deserialize_elem(XDR_schema *s,int fd, XDR_tree **out_t){
  ...
  case XDR_VARARRAY:
	...
	return rv;
      ...
      for(i =0;i<len;i++){
	if(XDR_OK!=(rv =XDR_deserialize_elem(((XDR_array*)s)->elem_type,
					    fd, ((XDR_tree_compound*)t)->subelems + i))){
  ...
  case XDR_STRUCT:
	...
	return rv;
      ...
      for(i = 0;i<len;i++){
	if(XDR_OK!=(rv=XDR_deserialize_elem(((XDR_struct *)s)->elems[i],fd
					    ,((XDR_tree_compound*)t)->subelems + i)))
	  return rv;
      }
