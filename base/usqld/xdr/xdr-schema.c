...
XDR_typedesc * XDR_schema_new_struct(size_t num_elems,const XDR_typedesc ** elems){
  ...
  r->elems = myllocn(XDR_typedesc*,num_elems);
  assert(r->elems!=NULL);
  memcpy(r->elems,elems,sizeof(XDR_typedesc *) * num_elems);
  r->num_elems = num_elems;
  return (XDR_typedesc*) r;
}


XDR_typedesc * XDR_schema_new_type_union(size_t num_alternatives,
...
const char * find_name(XDR_type t){
  int i;
...
void XDR_dump_schema_r(XDR_typedesc *elem,int indent){
  switch(elem->type){
  case XDR_INT:
  case XDR_UINT: 
  case XDR_ENUM: 
  ...
  case XDR_FIXEDOPAQUE:
    printf("%s[%d]\n",find_name(elem->type),((XDR_opaque*)elem)->num_elems);
    break;
  case XDR_FIXEDARRAY:
    printf("%s[%d]{\n",find_name(elem->type),((XDR_array*)elem)->num_elems);
    do_tab(indent+1);
  ...
  case XDR_UNION:
    {
      ...
      printf("%s(%d){\n",find_name(elem->type),((XDR_type_union*)elem)->num_alternatives);
...
void XDR_dump_schema( XDR_typedesc *elem){
