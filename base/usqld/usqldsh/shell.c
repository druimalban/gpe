...
*/
static int do_meta_command(char *zLine, usqld_conn *db, struct callback_data *p){
  int i = 1;
  int nArg = 0;
  int n, c;
  int rc = 0;
  ...
  */
  ...
  if( c=='r' && strncmp(azArg[0], "reindex", n)==0 ){
    ...
    char *zSql;
    if( nArg==1 ){
      rc = usqld_get_table(db,
        "SELECT name, sql FROM sqlite_master "
        "WHERE type='index'",
        &azResult, &nRow, 0, &zErrMsg
      );
    }else{
      rc = usqld_get_table_printf(db,
        "SELECT name, sql FROM sqlite_master "
        "WHERE type='index' AND tbl_name LIKE '%q'",
    ...
    for(i=1; rc==SQLITE_OK && i<=nRow; i++){
      extern char *usqld_mprintf(const char *, ...);
      zSql = usqld_mprintf(
         "DROP INDEX '%q';\n%s;\nVACUUM '%q';",
         azResult[i*2], azResult[i*2+1], azResult[i*2]);
      if( p->echoOn ) printf("%s\n", zSql);
      rc = usqld_exec(db, zSql, 0, 0, &zErrMsg);
    }
    usqld_free_table(azResult);
    if( zErrMsg ){
      fprintf(stderr,"Error: %s\n", zErrMsg);
      free(zErrMsg);
    }
  ...
  if( c=='s' && strncmp(azArg[0], "schema", n)==0 ){
    ...
    data.mode = MODE_Semi;
    if( nArg>1 ){
      if( strcasecmp(azArg[1],"sqlite_master")==0 ){
...
static void process_input(struct callback_data *p, FILE *in){
  ...
  while( fflush(p->out), (zLine = one_input_line(zSql, in))!=0 ){
    ...
    if( zSql/* && sqlite_complete(zSql) */){
      p->cnt = 0;
      if( usqld_exec(db, zSql, callback, p, &zErrMsg)!=0
           && zErrMsg!=0 ){
...
int main(int argc, char **argv){
  ...
  if( argc!=3 && argc!=4 ){
    fprintf(stderr,"Usage: %s ?OPTIONS? HOST DATABASE ?SQL?\n", Argv0);
    exit(1);
  }
  data.db = db = usqld_connect(argv[1],argv[2], &zErrMsg);
  if( db==0 ){
    if( zErrMsg ){
  ...
  if( argc==4 ){
    if( argv[2][0]=='.' ){
      ...
      int rc;
      rc = usqld_exec(db, argv[2], callback, &data, &zErrMsg);
      if( rc!=0 && zErrMsg!=0 ){
