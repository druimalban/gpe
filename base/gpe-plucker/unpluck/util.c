...
#define hashtable_hash_index(ht,key)	      (hash_string((key), (ht)->ht_size))
#define hashtable_compare_keys(ht,key1,key2)  (compare_strings((key1),(key2)))

static int compare_strings (char *key1, char *key2)
{
  return (strcmp(key1, key2) == 0);
}

static int hash_string (char * str, int size)
{
  unsigned long crc;
...
HashTable *_plkr_NewHashTable (int size)
{
  ...
  new->ht_size = size;
  new->ht_nPairs = 0;
  new->ht_slots = (HashTableSlot *) malloc(sizeof(HashTableSlot) * size);
