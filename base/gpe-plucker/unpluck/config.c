...
static HashTable *get_or_create_segment (char *name)
{
  HashTable *target;
...
static int read_config_file (char *filename)
{
  ...
  int linelen, len2, buf_index, status, line_number;
  FILE *fp = fopen(filename, "r");
  char *ptr, *str_end, *str_begin, *charptr, *current_option, *option_value;
  ...
  current_segment = get_or_create_segment ("default");
  current_option = NULL;
  ...
  while (1) {
    ...
    if (linebuf[buf_index] == SEGMENT_LEAD_CHAR) {
      if ((str_end = strchr(linebuf + buf_index + 1, SEGMENT_END_CHAR)) == NULL) {
      ...
      *str_end = 0;
      current_segment = get_or_create_segment (str_begin);
      /* fprintf (stderr, "Current segment is now %p (%s)\n", current_segment, str_begin); */
      if (current_option)
        free(current_option);
      current_option = NULL;

    } else if ((linebuf[0] == ' ' || linebuf[0] == '\t') && current_option != NULL) {
      /* continuation line */
...
static void
  try_read_config_file (char *dir, char *name)
{
  char *filename;
  ...
  initialize_config_info ()
{
  ...
  try_read_config_file(config_dir, system_config_file_name);
  if (home != NULL)
    try_read_config_file(home, user_config_filename);
}

char *
  plkr_GetConfigString (char *section_name, char *option_name, char *default_value)
{
  char *value = NULL;
  HashTable *section;
  ...
  if (section_name != NULL) {
    if ((section = (HashTable *) _plkr_FindInTable (SectionsTable, section_name)) != NULL)
      value = (char *) _plkr_FindInTable (section, option_name);
  }
  if (value == NULL && ((section_name == NULL) || (strcmp(section_name, "default") != 0))) {
    if ((section = (HashTable *) _plkr_FindInTable (SectionsTable, STRINGIFY(OS_SECTION_NAME))) != NULL)
      value = (char *) _plkr_FindInTable (section, option_name);
  }
  if (value == NULL && ((section_name == NULL) || (strcmp(section_name, "default") != 0))) {
    if ((section = (HashTable *) _plkr_FindInTable (SectionsTable, "default")) != NULL)
      value = (char *) _plkr_FindInTable (section, option_name);
  }
...
long int
  plkr_GetConfigInt (char *section_name, char *option_name, long int default_value)
{
...
double
  plkr_GetConfigFloat (char *section_name, char *option_name, double default_value)
{
  char *svalue = plkr_GetConfigString (section_name, option_name, NULL);
  char *endptr;
  double value;
  ...
  if (*endptr != 0) {
    _plkr_message ("Bad float value string '%s' for option %s:%s",
                   svalue, (section_name ? section_name : "default"), option_name);
    return default_value;
  } else {
    return value;
  }
...
int
  plkr_GetConfigBoolean (char *section_name, char *option_name, int default_value)
{
  char *svalue = plkr_GetConfigString (section_name, option_name, NULL);
  ...
  if ((strcmp(svalue, "1") == 0) ||
      ...
      (strcmp(svalue, "t") == 0) || (strcmp(svalue, "T") == 0) || (strcmp(svalue, "True") == 0))
    return 1;

  else if ((strcmp(svalue, "0") == 0) ||
           (strcmp(svalue, "false") == 0) || (strcmp(svalue, "FALSE") == 0) ||
           (strcmp(svalue, "off") == 0) || (strcmp(svalue, "OFF") == 0) ||
           (strcmp(svalue, "F") == 0) || (strcmp(svalue, "F") == 0) || (strcmp(svalue, "False") == 0))
    return 0;
